...
static void HandleHeaderButtonInteraction(Clay_ElementId elementId, const Clay_PointerData pointerData, void* userData) {
	if (pointerData.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME) {
		BF_Print("You pressed the button\n");
	}
}

static Clay_ElementDeclaration HeaderButtonStyle(const bool hovered) {
...
static void RenderHeaderButton(const Clay_String text) {
	CLAY_AUTO_ID(HeaderButtonStyle(Clay_Hovered())) {
		Clay_OnHover(HandleHeaderButtonInteraction, NULL);
...
void RightPanelRender(void) {
	CLAY(CLAY_ID("RightPanel"),
		{.layout = { .layoutDirection = CLAY_TOP_TO_BOTTOM, .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_GROW(0) }, .childGap = 16 }
	}) {
		CLAY_AUTO_ID({
			.layout = {
		...
		CLAY(CLAY_ID("MainContent"), {
			...
			CLAY(CLAY_ID("Photos2"), {
				...
				CLAY(CLAY_ID("Picture4"), {
					.layout = { .sizing = { .width = CLAY_SIZING_FIXED(120), .height = CLAY_SIZING_FIXED(120) }},
			...
			CLAY(CLAY_ID("Photos"), {
				.layout = { .sizing = { .width = CLAY_SIZING_GROW(0) }, .childAlignment = { .x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER },
				.childGap = 16, .padding = {16, 16, 16, 16} }, .backgroundColor = {180, 180, 220, 255}
			}) {
				CLAY(CLAY_ID("Picture2"), { .layout = { .sizing = { .width = CLAY_SIZING_FIXED(120) }}, .aspectRatio = 1, .image = { .imageData = getProfilePicture() }}) {}
				CLAY(CLAY_ID("Picture1"), {
					.layout = { .childAlignment = { .x = CLAY_ALIGN_X_CENTER }, .layoutDirection = CLAY_TOP_TO_BOTTOM, .padding = {8, 8, 8, 8} },
				...
				CLAY(CLAY_ID("Picture3"), { .layout = { .sizing = { .width = CLAY_SIZING_FIXED(120) }}, .aspectRatio = 1, .image = { .imageData = getProfilePicture() }}) {}
			}
